...
float map(vec3 p) {
  float z = 20.;
  p.xz = mod(p.xz-z*.5, z)-z*.5;
  float hash = random(floor((p.xz-z*.5)/z));
  float no = noise(vec3(p.y*.2+time*hash,1.+hash*54.109,1.));
  float seed=0.;
  p.xz *= rot(p.y*.2);
  p.xz *= rot(no*4.);
  p.z -= 1.+no*4.;
  p.xz *= rot(p.y);
  float r = sdThread(p, random(12.12+hash*34.51));
  for(int i=0; i<=4; i++) {
    seed += 1.;
    p.x *= 1.4 + 1.;
    p.xz *= rot(2.);
    r = smin(r, sdThread(p, random(seed+hash)), 1.2);
  }
  return r/10.;
}
