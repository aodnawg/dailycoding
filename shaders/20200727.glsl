...
float map(vec3 p) {
  p.x += 1.;
  p.xz = pmod(p.xz, 10.);
  p.xz *= rot(4.5+noise(p+vec3(time))*.1);
  p.xy = pmod(p.xy, 10.);
  float n = noise(p+vec3(time))*10.+10.;
  float result = length(p.xz)-min(1., max(-1., cos(length(p)/n)));
...
void main(void) {
  ...
  col = vec3(step/1000.);
  col = pow(col, vec3(2.5)+pow(sin(time*2.), 2.));
