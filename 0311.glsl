...
float film(vec3 p) {
	float z = .5;
    vec2 F = cellular(vec3(p*z+vec3(iTime,0,0)));
...
float map(vec3 p, vec3 ro) {
    float glitch = step(.9, random(floor(iTime*2.)))*noise(p)*1.;

    // p.xz = mod(p.xz, 10.) - 5.;
    float z = 1. + sin(iTime)*.5;
	float n = film(p);
	return n;
    float r = 2.;
    float s = abs(length(p)-r-glitch)-.01;
    return min(max(s, n-.3), length(p)-r+cos(iTime)*.02+.04+glitch*.4);
}
